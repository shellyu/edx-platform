...
def upload_file(file, location):
    filename = 'subs_{}.srt.sjson'.format(get_subs_id(file.name))
...
class TestVideo(BaseTestXmodule):
    ...
    def tearDown(self):
        clear_assets(self.item_module.location)
...
class TestVideoGetTranscriptsMethod(TestVideo):
    ...
    def test_good_transcript(self):
        ...
        good_sjson = create_file(content="""
                {
                  "start": [
                    270,
                    2720
        ...
        upload_file(good_sjson, self.item_module.location)
        subs_id = get_subs_id(good_sjson.name)
